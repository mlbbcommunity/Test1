...
class WhatsAppBot {
        ...
        if (connection === 'close') {
            ...
            if (shouldReconnect) {
                ...
                if (isConnectionFailure || isQRFailure || isAuthFailure) {
                    logger.warn('Authentication/Connection failure detected - forcing pairing code generation');
                    
                    // Clear session data for fresh authentication
                    await this.clearSessionData();
                    this.qrRetries = 0;
                    
                    // Force immediate pairing code generation
                    setTimeout(async () => {
                        logger.info('Forcing pairing code generation...');
                        await this.connectToWhatsApp();
                        // Give it a moment then force pairing code
                        setTimeout(async () => {
                            await this.generatePairingCode();
                        }, 2000);
                    }, 1000);
    ...
    async generatePairingCode() {
        try {
            ...
            // Clean and validate phone number - try multiple formats
